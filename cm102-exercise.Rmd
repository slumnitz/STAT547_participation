...
```{r}
str_split(fox, pattern=" ")[[1]] %>% length() # output is always a list
...
NOTES: l[[1]] -> double brackets give me the single argument at this place
[1] -> give me the whole thing
list in R similar to dictionary
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>% # at least 7 characters, add more if not using $
    head()
```
...
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ") # takes output and coombines them separated by a , -> returns a vector of length 1
```
...
```{r}
str_view_all_match(countries, pattern="[[ae][ae]]{2}") # (a|e)(a|e), (a|e){2}
```
...
str_view_all_match(countries, pattern = "$") # finds the end of each line
```

Countries that start with "Ca":

```{r}
str_view_all_match(countries, pattern = "^Ca") # ^ = start
```
...
```{r}
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\1") # I want an a followed by a single b, and what was resolved in group one, so aba
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\2") # abb
```

Note that the parentheses are first resolved, THEN referred to. NOT re-executed.

```{r}
str_view_all(c("bananas"), "(.)(.)\\1\\2.*\\1\\2") anything followed by anything, repeat first and second grop, have any number of characters and gepead first and second group
```

We can refer to them later in the search, too:

```{r}
str_view_all(c("bananas", "Who can? Bananas can."), "(.)(.)\\1\\2.*\\1\\2") # advanced pattern recognition
```
